...
const { siteRoot, siteURL, siteTitle, siteFacebookAdmins, siteFacebook, siteTwitter } = Info
...
export default ({
  ...
  return (
    <Head>
      ...
      {videos &&
        videos.slice(0, 6).map(video => <meta key={video} property="og:video" content={video} />)}
      {seriesPaths &&
        seriesPermalinks
          .slice(0, 6)
